...
int main(void)
{
    ...
    puts("Doubly Linked List Testing Succeeded"); 
...
status_t insert_start(list_t* p_list, data_t new_data)
{
    ...
    //  i.e. p_list->next->prev must be set 
    //  before p_list->next is set to new_node 
...
status_t insert_end(list_t* p_list, data_t new_data) 
{
    ...
    new_node->next = NULL;  //  This is already NULL via get_node()
                            //  yet keeping it here for the sake of understanding 
    new_node->prev = run; 
...
status_t pop_start(list_t* p_list, data_t* p_start_data)
{
    ...
    node_t* r_node = p_list->next; 

    *p_start_data = r_node->data; 
...
status_t append(list_t* p_list_1, list_t** pp_list_2)
{
    ...
    {
        node_t* run1 = p_list_1; 
        while(run1->next != NULL)
            run1 = run1->next; 

        run1->next = p_list_2->next; 
        p_list_2->next->prev = run1; 
    }
...
status_t reverse(list_t* p_list)
{
    ...
    if(is_list_empty(p_list) || p_list->next->next == NULL)
        return (SUCCESS); 
